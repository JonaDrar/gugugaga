...
def _recipes():
    ...
    r["head-miku"] = {
        ...
        "prompt": (
            "Using the FIRST attached image as the reference, generate ONLY THE FRONT PART "
            "of a turquoise hair wig, to be placed OVER a character's face. "
            "It has TWO parts, and BOTH are equally important. "
            "(1) TWO THICK SIDE LOCKS of turquoise hair, one on each side of the face, "
            "hanging straight down PAST THE CHIN. Each side lock must be as wide and reach "
            "as far out to the side as the corresponding side of the hood in the reference "
            "image, at every height, covering the whole area between the face and the "
            "hood's outer edge. "
            "(2) A VERY LONG, THICK, HEAVY BLUNT FRINGE joining the two side locks across "
            "the top. This is the part that usually comes out too short, so make it "
            "clearly longer than feels natural: it starts at the very top of the piece and "
            "hangs straight DOWN, covering the whole forehead AND the entire UPPER HALF of "
            "the reference hood's face opening, stopping only at eyebrow level. "
            "As a proportion: the fringe alone must take up about the top 45% of the total "
            "height of the whole piece. The remaining opening below it is a wide, short "
            "horizontal gap — only eyes, nose and mouth fit in it, no forehead at all. "
            "Between the fringe and the two side locks there is an opening — smaller than "
            "the hood's because the fringe hangs into it — and that area stays EMPTY and "
            "transparent, because that is where the face shows through. "
            "Do NOT draw the back or the top-back of the hair, and do NOT draw twintails: "
            "those are a separate image. "
            "Include a black futuristic headset over the ears with a hot-pink stripe and a "
            "small microphone arm curving toward the mouth. "
            "Use the SECOND attached image only as a colour and style guide. "
            "No face, no head, no character, no body. 3D Nendoroid figurine style, glossy "
            "toy look, soft frontal lighting. Isolated object, centered, transparent "
            "background, no shadow, no text."
        ),
    }
    r["body-miku"] = {
        ...
        "prompt": (
            ...
            "them. "
            "The figure stands with its LEGS TOGETHER: the two boots are side by side and "
            "TOUCH each other along their inner edges from the skirt down to the ground, "
            "leaving NO opening, NO hole and NO empty space between the legs — the whole "
            "area below the skirt is solid black boot. Nothing at all may be visible "
